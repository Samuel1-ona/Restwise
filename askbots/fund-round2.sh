#!/bin/bash
# Fund the AskBots ROUND TWO review for Restwise.
#
# Uses askbots/round2.json: questions are byte-identical to round one -- the track scores
# the gap, so the instrument must not change. Only the project label differs, because the
# CLI resumes any run whose submission document it has already seen.
#
...
ASKBOTS_PRIVATE_KEY="$KEY" npx --yes askbots@0.2.0 submit \
  --file askbots/round2.json --execute --json
